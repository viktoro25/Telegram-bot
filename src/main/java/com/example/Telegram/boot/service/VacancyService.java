package com.example.Telegram.boot.service;
...
public class VacancyService {
    ...
    public List<VacancyDto> getJuniorVacancies() {
        ...
        return vacancies.values().stream().filter(v -> v.getTitle().toLowerCase().contains("middle"))
                .toList();
    }public List<VacancyDto> getSeniorVacancies() {
        return vacancies.values().stream().filter(v -> v.getTitle().toLowerCase().contains("senior"))
                .toList();
    }
